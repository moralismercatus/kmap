/******************************************************************************
 * Author(s): Christopher J. Havlicek
 *
 * See LICENSE and CONTACTS.
 ******************************************************************************/
#include "option.hpp"

#include "cmd/command.hpp"
#include "cmd/parser.hpp"
#include "contract.hpp"
#include "error/master.hpp"
#include "error/parser.hpp"
#include "io.hpp"
#include "js_iface.hpp"
#include "path/node_view.hpp"

#include <boost/variant.hpp>

#include <emscripten.h>

namespace kmap {

OptionStore::OptionStore( Kmap& kmap )
    : kmap_{ kmap }
{
}

auto OptionStore::option_root()
    -> Uuid
{
    return KMAP_TRYE( kmap_.fetch_or_create_descendant( "/meta.setting.option" ) );
}

auto OptionStore::install_option( Heading const& heading
                                , std::string const& descr
                                , std::string const& value 
                                , std::string const& action  )
    -> Result< Uuid >
{
    auto rv = KMAP_MAKE_RESULT( Uuid );
    auto const action_body = fmt::format( "```javascript\n{}\n```", action );

    KMAP_ENSURE( cmd::parser::parse_body_code( action_body ), error_code::parser::parse_failed );

    auto const vopt = view::root( option_root() )
                    | view::direct_desc( heading );

    KMAP_TRY( kmap_.update_body( KMAP_TRY( vopt | view::direct_desc( "description" ) | view::fetch_or_create_node( kmap_ ) )
                               , descr ) );
    KMAP_TRY( kmap_.update_body( KMAP_TRY( vopt | view::direct_desc( "value" ) | view::fetch_or_create_node( kmap_ ) )
                               , value ) );
    KMAP_TRY( kmap_.update_body( KMAP_TRY( vopt | view::direct_desc( "action" ) | view::fetch_or_create_node( kmap_ ) )
                               , action_body ) );

    rv = KMAP_TRY( vopt | view::fetch_node( kmap_ ) );

    return rv;
}

auto OptionStore::apply( Uuid const& option )
    -> Result< void >
{
    auto rv = KMAP_MAKE_RESULT( void );

    // TODO: KMAP_ENSURE( is_option( opt ) );

    io::print( "Option found: {}\n", kmap_.absolute_path_flat( option ) );
    // auto const action = KMAP_TRY( view::make_view( kmap_, option ) | view::child( "action" ) | view::try_to_single );
    auto const action = KMAP_TRY( kmap_.fetch_child( option, "action" ) );
    // auto const action_body = KMAP_TRY( action.fetch_body() );
    auto const action_body = KMAP_TRY( kmap_.fetch_body( action ) );
    auto const code = cmd::parser::parse_body_code( action_body ).value();
    auto const value_node = KMAP_TRY( kmap_.fetch_child( option, "value" ) );
    auto const value_body = KMAP_TRY( kmap_.fetch_body( value_node ) );
    // TODO: Use similar technique to cmd action to expose "option_value" automatically via fn arg.
    boost::apply_visitor( [ & ]( auto const& e ) // Note: spirit::x3::variant uses boost::variant which is not compatible with std::visit.
                            {
                                using T = std::decay_t< decltype( e ) >;

                                if constexpr( std::is_same_v< T, cmd::ast::Kscript > )
                                {
                                    KMAP_THROW_EXCEPTION_MSG( "kscript is not supported in a guard node" );
                                }
                                else if constexpr( std::is_same_v< T, cmd::ast::Javascript > )
                                {
                                    auto const opt_val = fmt::format( "let option_value = {};", value_body );

                                    KMAP_TRYE( js::eval_void( fmt::format( "{}\n{}", opt_val, e.code ) ) );
                                }
                                else
                                {
                                    static_assert( always_false< T >::value, "non-exhaustive visitor!" );
                                }
                            }
                        , code );
    
    rv = outcome::success();

    return rv;
}

auto OptionStore::apply_all()
    -> Result< void >
{
    auto rv = KMAP_MAKE_RESULT( void );
    auto const root = option_root();
    auto const options = view::root( root )
                       | view::desc
                       | view::child( view::all_of( "description", "action" ) ) // Select nodes when children ("description", "action")
                       | view::parent
                       | view::to_node_set( kmap_ ); // Select parent of ("description", "action")

    for( auto const& option : options )
    {
        KMAP_TRY( apply( option ) );
    }

    rv = outcome::success();

    return rv;
}

namespace apply_options_def {

auto const guard_code =
R"%%%(```javascript
return kmap.success( 'success' );
```)%%%";
auto const action_code =
R"%%%(```javascript
kmap.apply_options();

return kmap.success( 'all options applied' );
```)%%%";

using Guard = PreregisteredCommand::Guard;
using Argument = PreregisteredCommand::Argument;

auto const description = "applies all options in /meta.option";
auto const arguments = std::vector< Argument >{};
auto const guard = Guard{ "unconditional"
                        , guard_code };
auto const action = action_code;

REGISTER_COMMAND
(
    apply.options
,   description 
,   arguments
,   guard
,   action
);

} // namespace apply_options_def

} // namespace kmap


